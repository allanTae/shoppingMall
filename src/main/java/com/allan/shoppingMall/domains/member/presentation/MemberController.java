...
public class MemberController {
    ...
    public String join(@ModelAttribute("memberForm") @Valid MemberForm form, BindingResult bindingResult, HttpServletResponse response){
        ...
        log.info("postCode: " + form.getPostCode());
        ...
        if(!Pattern.compile(ID_PATTERN).matcher(form.getAuthId().trim()).find()){
        ...
        if(!Pattern.compile(NAME_PATTERN).matcher(form.getName().trim()).find()){
        ...
        if(!Pattern.compile(PASSWORD_PATTERN).matcher(form.getPwd().trim()).find()){
        ...
        if(!bindingResult.hasErrors() && !form.getPwd().equals(form.getRePwd().trim())){
        ...
        if(!Pattern.compile(EMAIL_PATTER).matcher(form.getEmail().trim()).find()){
        ...
        if(!Pattern.compile(PHONE_PATTERN).matcher(FormatUtil.phoneFormat(form.getPhone().trim())).find()){
            log.error("memberController's join() fail email validation check!!");
            bindingResult.rejectValue("phone", "phone.invalidatedVal", "올바른 휴대전화 형식이 아닙니다.");
        }
