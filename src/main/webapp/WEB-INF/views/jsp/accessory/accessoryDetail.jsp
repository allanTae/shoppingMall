...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
    ...
    <div class="container itemContainerWrap col-md-12">
        <div class="itemContainer row">
            <div class="imageContainer col-md-6 m-0">
                <img class="mainProfileImage col-md-12 p-2 img-fluid" src=<c:out value="${pageContext.request.contextPath}/image/${accessoryInfo.previewImages[0]}" /> alt="">
                <ul class="col-md-12 row pr-3" style="list-style: none;">
                    <c:forEach var="image" items="${accessoryInfo.previewImages}" varStatus="index">
            ...
            <div class="goodsForm col-md-6 p-4">
                    ...
                    <div class="pt-3">
                        <div class="form-group row m-0">
                            <table class="table col-md-12" id="fabricTable" style="border: 1px solid black;">
                              ...
                              <tbody class="border-0">
                                <c:forEach var="itemFabric" items="${accessoryInfo.itemFabrics}" varStatus="index" >
                                    <tr>
                        ...
                        <div class="form-group row m-0">
                            <table class="table col-md-12" id="detailTable" style="border: 1px solid black;">
                              ...
                              <tbody class="border-0">
                                <c:forEach var="itemDetail" items="${accessoryInfo.itemDetails}" varStatus="index" >
                                    <tr>
                ...
                <div class="chooseOption">
                    ...
                    <select class="form-select m-0" id="accessorySizeSelect" aria-label="Default select example" >
                      <option selected>사이즈를 선택 해 주세요.(필수)</option>
                      <c:forEach var="accessorySize" items="${accessoryInfo.accessorySizes}" varStatus="index" >
                        <option value="${index.count}">${accessorySize.sizeLabel}</option>
                      </c:forEach>
                    </select>
                    <p class="m-0"><br /></p>
                </div>
        ...
        <div class="imageContainerWrap row">
            <div class="detailImages col-md-12">
                <c:forEach var="image" items="${accessoryInfo.detailImages}" varStatus="index">
    ...
    <script>
      ...
      $("#accessorySizeSelect").on("change", function(event){
      ...
      function checkSelectOptions(){
        if(orderSizeCheck === true){
            ...
            // selected text.
            var selectedSize = $("select[id=accessorySizeSelect] option:selected").text();
            var selectedColor = "${accessoryInfo.color}";
            ...
            $("#accessorySizeSelect option:eq(0)").prop("selected", true);
            orderSizeCheck = false;
        }
      }
      ...
      function addSelectedItem(selectedSize,selectedColor,selectedOptionInfo){
        ...
        htmls += '    <span class="col-md-4 text-end align-center"><fmt:formatNumber type="number" maxFractionDigits="3" value="${accessoryInfo.price}" />원</span>';
        htmls += '  </div>';
        htmls += '</div>';
        $("#selectedClothesArea").append(htmls);
      }

      let defaultAmount = ${accessoryInfo.price};
      let totalAmount = 0;
      ...
      function getOrderForm(){
          ...
          orderFormInfo += '<input type="text" name="orderItems[0].itemId" value="${accessoryInfo.accessoryId}" />';
          orderFormInfo += '<input type="text" name="orderItems[0].itemName" value="${accessoryInfo.accessoryName}" />';
          orderFormInfo += '<input type="text" name="orderItems[0].previewImg" value="${accessoryInfo.previewImages[0]}" />';
          orderFormInfo += '<input type="text" name="orderItems[0].price" value="${accessoryInfo.price}" />';
